import 'package:ajmanbank/Viewer/DashBoard/DashBoardTabBarScreen/Processed/processedcontroller.dart';
...
class ProcessedScreen extends StatefulWidget {
  const ProcessedScreen({Key? key}) : super(key: key);
...
class _ProcessedScreenState extends State<ProcessedScreen> {
  late ProcessedController _controller;
  ...
  void initState() {
    ...
    super.initState();
    _controller = Get.put(ProcessedController());
  ...
  Future<void> _scrollListener() async {
    ...
    if (maxScroll - currentScroll <= _scrollThreshold) {
      if (_controller.currentPage <= _controller.totalPages && !_controller.status.isLoadingMore) {
        await _controller.processList(_controller.fromDate, _controller.toDate, false,true,null,null);
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return GetBuilder<ProcessedController>(builder: (logic) {
      return Padding(
        ...
        child: Column(
          children: [
            ...
            SizedBox(height: 20),
            Expanded(
              child: (_controller.ProcessListSize != 0)
                  ? ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    String? ApprovedOnTimeString = item.approvedOn;
                    DateTime approvedOn = DateTime.parse(ApprovedOnTimeString!);
                    return Column(
                      children: [
                        ExpansionTile(
                          title: Column(
                                ...
                                Padding(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        '${approvedOn.toLocal().toString().split('.')[0]}',
                                        style: TextStyle(
                                          fontFamily: MyFont.myFont,
                                        ),
                                      ),
